(library (state-tests)
  ...
  (define (run-state-tests)
    ...
    (tassert "unify ground-ground" (unify empty-state 2 2) empty-state)
    (tassert "unify free-ground" (walk (unify empty-state x0 2) x0) 2)
    (tassert "unify free-ground" (walk (unify empty-state 2 x0) x0) 2)
    (tassert "unify variable to itself" (unify empty-state x0 x0) empty-state)
    (tassert "unify free-free" (walk (unify empty-state x0 x2) x0) x2)
    (tassert "unify free-free inverse priority" (walk (unify empty-state x2 x0) x0) x2)
    (tassert "unify cars" (walk (unify empty-state (cons x0 0) (cons 2 0)) x0) 2)
    (tassert "unify cars fail" (unify empty-state (cons 0 x0) (cons 2 0)) failure)
    (tassert "unify cdrs" (walk (unify empty-state (cons 0 x0) (cons 0 2)) x0) 2)
    (tassert "unify cdrs fail" (unify empty-state (cons x0 2) (cons 2 0)) failure)))
