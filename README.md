...
- make == pre-unify with an empty mini-substitution to catch failing ground violations. useful if we want to quick check the constraints with new extensions (build/preprocessed/datatypes.ss:112)
- delete goal-cond (build/preprocessed/datatypes.ss:124)
- replace conj with make-conj where possible (build/preprocessed/datatypes.ss:131)
- evaluate divergence avoidance in conj goals (build/preprocessed/datatypes.ss:136)
- is conj-fold ever used? (build/preprocessed/datatypes.ss:148)
...
- is it possible to use the delta on == as a minisubstitution and totally ignore the full substitution when checking constraints? maybe we only have to start doing walks when we reach the simplification level where vars wont be in lowest terms (build/preprocessed/solver.ss:25)
- quick replace extended vars in constraints looked up during unify and check for immediate failures (build/preprocessed/solver.ss:26)
- consider making occurs check a goal that we can append in between constraints we find and the rest of the ctn, so it only walks if constraints dont fail (build/preprocessed/solver.ss:27)
- disunification unifier can be small step: we nly need to know 1 =/= succeeds before proceeding with search (build/preprocessed/solver.ss:36)
- =/= may not need to fire all the constraints conjoined to a given attributed var. maybe only grab a subset with == in them somewhere (build/preprocessed/solver.ss:54)
- break fail and succeed into separate cases (build/preprocessed/solver.ss:78)
- if we return the state, we should also strip the goal of conjuncts so we dont double tap (build/preprocessed/solver.ss:81)
- replace disj with make-disj where possible (build/preprocessed/solver.ss:90)
- succeed should probably skip any computations in diff-== (build/preprocessed/solver.ss:98)
- be more specific about how many disjuncts we need attr vars from (build/preprocessed/solver.ss:132)
- reevaluate inverting disj given that they are now binary (build/preprocessed/solver.ss:137)
- perhaps instead of a fully inverted disj constraint pair we can simply add a dummy proxy constraint that if looked up succeeds but raises the constraint waiting on the original vars (build/preprocessed/solver.ss:140)
- optimize which disj constraint we pick for attribution to minimize free vars (build/preprocessed/solver.ss:151)
- if we are checking 2 disjuncts, do we need both attr vars? (build/preprocessed/solver.ss:154)
...
- make == pre-unify with an empty mini-substitution to catch failing ground violations. useful if we want to quick check the constraints with new extensions (src/aikanren/datatypes.ss:117)
- delete goal-cond (src/aikanren/datatypes.ss:129)
- replace conj with make-conj where possible (src/aikanren/datatypes.ss:136)
- evaluate divergence avoidance in conj goals (src/aikanren/datatypes.ss:142)
- is conj-fold ever used? (src/aikanren/datatypes.ss:156)
...
- quick replace extended vars in constraints looked up during unify and check for immediate failures (src/aikanren/solver.ss:29)
- consider making occurs check a goal that we can append in between constraints we find and the rest of the ctn, so it only walks if constraints dont fail (src/aikanren/solver.ss:30)
- disunification unifier can be small step: we nly need to know 1 =/= succeeds before proceeding with search (src/aikanren/solver.ss:39)
- =/= may not need to fire all the constraints conjoined to a given attributed var. maybe only grab a subset with == in them somewhere (src/aikanren/solver.ss:58)
- break fail and succeed into separate cases (src/aikanren/solver.ss:83)
- if we return the state, we should also strip the goal of conjuncts so we dont double tap (src/aikanren/solver.ss:86)
- replace disj with make-disj where possible (src/aikanren/solver.ss:95)
- succeed should probably skip any computations in diff-== (src/aikanren/solver.ss:103)
- be more specific about how many disjuncts we need attr vars from (src/aikanren/solver.ss:139)
- reevaluate inverting disj given that they are now binary (src/aikanren/solver.ss:144)
- perhaps instead of a fully inverted disj constraint pair we can simply add a dummy proxy constraint that if looked up succeeds but raises the constraint waiting on the original vars (src/aikanren/solver.ss:147)
- optimize which disj constraint we pick for attribution to minimize free vars (src/aikanren/solver.ss:158)
- if we are checking 2 disjuncts, do we need both attr vars? (src/aikanren/solver.ss:162)
