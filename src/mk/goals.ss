...
;	      (org-display (print-substitution s) (print-reification s))
  ...
  ;; === TRACING INTERPRETER ===
    ...
    (cond
     ...
     [(conde? g) (let*-values ([(lhs p) (trace-run-goal (conde-lhs g) s p (if (conde? (conde-lhs g)) depth (fx1- depth)))]
			       [(rhs p) (trace-run-goal (conde-rhs g) s p (if (conde? (conde-rhs g)) depth (fx1- depth)))])
