...
;    (tassert "=/= & (fail|succeed)" (run1 (x1 x2) (disj (conj (=/= x1 1) (== x2 2)) (== x1 1)) (=/= x1 1)) (list (=/= x1 1) 2))
    ...
    (tassert "== succeed" (simplify-=/=2 (== x1 1) x1 1) (list succeed fail fail))
    (tassert "== undecidable" (simplify-=/=2 (== x1 (cons x2 x3)) x1 (cons x3 x2)) (list (== x1 (cons x2 x3)) (== x1 (cons x2 x3)) succeed))
    (tassert "=/= undecidable" (simplify-=/=2 (=/= x1 (cons x2 x3)) x1 (cons x3 x2)) (list (=/= x1 (cons x2 x3)) (=/= x1 (cons x2 x3)) succeed))
    (tassert "symbolo fail" (simplify-=/=2 (symbolo x1) x1 1) (list fail (symbolo x1) succeed))
    (tassert "symbolo2 undecidable" (simplify-=/=2 (symbolo x1) x2 1) (list (symbolo x1) (symbolo x1) succeed))
    (tassert "not numbero fail" (simplify-=/=2 (noto (numbero x1)) x1 1) (list fail (noto (numbero x1)) succeed))
    (tassert "=/= fail" (simplify-=/=2 (=/= x1 1) x1 1) (list fail (=/= x1 1) succeed))
    (tassert "=/= succeed" (simplify-=/=2 (=/= x1 2) x1 1) (list succeed (=/= x1 2) succeed))
    (tassert "conj fail" (simplify-=/=2 (conj (== x1 1) (=/= x1 1)) x1 1) (list fail fail fail))
    (tassert "matcho fail" (simplify-=/=2 (matcho ([x1 (a . d)])) x1 1) (list fail succeed fail))
    (tassert "matcho2 undecidable" (simplify-=/=2 (matcho ([x1 (a . d)])) x2 1) (lambda (a) (and (matcho? (car a)) (matcho? (cadr a)) (succeed? (caddr a)))))
    (tassert "not matcho succeed" (simplify-=/=2 (noto (matcho ([x1 (a . d)]))) x1 1) (lambda (a) (and (succeed? (car a)) (noto? (cadr a)) (succeed? (caddr a)))))
    ...
    (tassert "=/= satisfied|satisfied|unnormalized" (simplify-=/=2 (disj (=/= x1 1) (disj (symbolo x1) (=/= x2 2))) x1 1) (list (=/= x2 2) (disj (=/= x1 1) (disj (symbolo x1) (conj (=/= x1 1) (=/= x2 2)))) succeed))
    (tassert "=/= unsatisfiable|satisfied" (simplify-=/=2 (disj (== x1 1) (symbolo x1)) x1 1) (list succeed (symbolo x1) succeed))
    (tassert "=/= satisfied|unsatisfiable" (simplify-=/=2 (disj (symbolo x1) (== x1 1)) x1 1) (list succeed succeed (symbolo x1)))
    (tassert "=/= satisfied|unsatisfiable|undecidable" (simplify-=/=2 (disj (symbolo x1) (disj (== x1 1) (=/= x1 2))) x1 1) (list succeed succeed (disj (symbolo x1) (conj (=/= x1 1) (=/= x1 2)))))
    (tassert "=/= satisfied|undecidable" (simplify-=/=2 (disj (symbolo x1) (=/= x1 2)) x1 1) (list succeed (disj (symbolo x1) (conj (=/= x1 1) (=/= x1 2))) succeed))
    (tassert "=/= unsatisfiable|undecidable" (simplify-=/=2 (disj (== x1 1) (=/= x1 2)) x1 1) (list succeed succeed (conj (=/= x1 1) (=/= x1 2))))
    (tassert "=/= undecidable|unsatisfiable" (simplify-=/=2 (disj (=/= x1 2) (== x1 1)) x1 1) (list (conj (=/= x1 1) (=/= x1 2)) succeed (conj (=/= x1 1) (=/= x1 2))))
    (tassert "=/= undecidable|undecidable" (simplify-=/=2 (disj (=/= x1 2) (=/= x1 3)) x1 1) (list succeed (conj (=/= x1 1) (disj (=/= x1 2) (=/= x1 3))) succeed))
