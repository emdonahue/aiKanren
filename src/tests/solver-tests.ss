(library (solver-tests) ; Tests the core mechanisms of the constraint solver
  ...
  (define (run-solver-tests)
    ...
    (tassert "constraint == ==|fail" (run1 (x1 x2) (constraint (== x1 1)) (constraint (conde ((== x2 2)) ((== x1 2))))) '(1 2))
    (tassert "constraint == ==|succeed" (run1 (x1 x2) (constraint (== x1 1)) (constraint (conde ((== x2 2)) ((== x1 1))))) (list 1 x2))
    (tassert "constraint == ==|succeed(out)" (run1 (x1 x2 x3) (constraint (== x1 1)) (constraint (== x3 3) (conde ((== x2 2)) ((== x1 1))))) (list 1 x2 3))
    (tassert "constraint == ==|==|=="
    ...
    (tassert "disj only walks 1st disjunct if no ==" (run1 (x1 x2) (== x2 2) (constraint (conde [(=/= x1 1)] [(=/= x2 2)]))) (list (=/= x1 1) 2))
    #;
    (parameterize ([lazy-solver #t])
    ...
    (tassert "disunify simplify matcho" (run1 (x1) (constraint (matcho ([x1 (a . d)]))) (=/= x1 '(1 . 2))) (lambda (c) (and (conj? c) (equal? (conj-lhs c) (=/= x1 '(1 . 2))) (matcho? (conj-rhs c)))))
    (tassert "disunify simplify negative matcho" (run1 (x1) (constraint (noto (matcho ([x1 (a . d)])))) (=/= x1 1)) (lambda (c) (and (conj? c) (matcho? (noto-goal (conj-rhs c))) (equal? (conj-lhs c) (=/= x1 1)))))
...
;;(tassert "matcho doesnt blend" (caddr (run1 (x1 x2 x3) (== x1 (cons x2 x3)) (absento 'closure x1))) 1)
    ...
    (tassert "reduce == partitions ctn and recheck" (run1 (x1 x2 x3 x4) (== x1 (cons x2 3)) (=/= x2 x4) (constraint (noto (matcho ([x1 (a . b)]) (== a x3))))) (list (cons (conj (=/= x2 x4) (=/= x2 x3)) 3) (conj (=/= x2 x4) (=/= x2 x3)) x3 x4))
